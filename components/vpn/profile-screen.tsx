...
export function ProfileScreen({ onNavigate }: ProfileScreenProps) {
  ...
  const [newReissueUrl, setNewReissueUrl] = useState<string | null>(null)
  ...
  async function handleReissueLink() {
    ...
    try {
      const res = await fetch('/api/vpn/reissue', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData?.error || 'Ошибка перевыпуска ссылки')
      }

      const data = await res.json()
      setReissueMessage(data.message || 'Ссылка подписки успешно перевыпущена!')
      setNewReissueUrl(data.subscription_url || null)
      setConfirmReissue(false)
      ctx.refreshUser()
    } catch (err) {
      setReissueError(err instanceof Error ? err.message : 'Не удалось перевыпустить ссылку')
    } finally {
  ...
  return (
    <div style={{ position:'relative', width:'100%', height:'100%', display:'grid', gridTemplateRows:'auto 1fr', background:C.bg, overflow:'hidden' }}>
      ...
      <main style={{ overflowY:'auto', scrollbarWidth:'none' as any, padding:`8px ${R.padH} 16px`, display:'flex', flexDirection:'column', gap:'10px', overscrollBehavior:'contain' }}>
              ...
              <div style={{ borderRadius:R.cardRadius, background:C.successSoft, overflow:'hidden', ...reveal(mounted,2) }}>
                ...
                <div style={{ padding:`10px ${R.cardPadH}`, display:'flex', flexDirection:'column', gap:'8px' }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                    <p style={{ fontFamily:BODY, fontSize:'11px', fontWeight:700, color:C.textMuted, margin:0 }}>Использовано:</p>
                    <div style={{ display:'flex', justifyContent:'space-between', gap:'10px', alignItems:'center' }}>
                      <span style={{ fontFamily:BODY, fontSize:'13px', fontWeight:600, color:C.text }}>🌐 Обычные сервера</span>
                      <span style={{ fontFamily:DISPLAY, fontSize:'14px', fontWeight:800, color:C.success }}>∞</span>
                    </div>
                    <div style={{ display:'flex', justifyContent:'space-between', gap:'10px', alignItems:'center' }}>
                      <span style={{ fontFamily:BODY, fontSize:'13px', fontWeight:600, color:'#4A6EF5' }}>🏳️ Белые списки</span>
                      {(() => {
                        const wlUsed = (sub as any)?.whitelist_traffic_used ?? 0
                        const wlLimit = (sub as any)?.whitelist_traffic_limit ?? 0
                        const wlUsedMb = Math.round(wlUsed / (1024 * 1024))
                        const wlLimitGb = Math.round(wlLimit / (1024 * 1024 * 1024))
                        return (
                          <span style={{ fontFamily:DISPLAY, fontSize:'14px', fontWeight:800, color:'#4A6EF5' }}>
                            {wlUsedMb} МБ / {wlLimitGb} ГБ
                          </span>
                        )
                      })()}
                    </div>
                    ...
                    <div style={{ borderTop:`1px solid rgba(0,0,0,0.08)`, padding:`10px ${R.cardPadH}`, display:'flex', flexDirection:'column', gap:'6px' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: '#FFE9E9', border:'1.5px solid #FFB3B3', borderRadius:'8px', padding:'10px 12px',
                          color:'#B02020', fontFamily:BODY, fontSize:'12px', fontWeight:700, cursor: reissueLoading ? 'not-allowed' : 'pointer',
                      ...
                      {reissueMessage && (
                        <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                          <p style={{ fontFamily:BODY, fontSize:'11px', color:C.success, margin:'0' }}>{reissueMessage}</p>
                          {newReissueUrl && (
                            <div style={{ padding:'10px', borderRadius:'12px', background:'rgba(255,255,255,0.08)' }}>
                              <p style={{ fontFamily:BODY, fontSize:'11px', color:'#fff', margin:'0 0 4px', fontWeight:700 }}>Все предыдущие подключения отозваны.</p>
                              <p style={{ fontFamily:BODY, fontSize:'11px', color:'#D1D5DB', margin:'0 0 8px' }}>💎 Новая ссылка:</p>
                              <p style={{ fontFamily:'monospace', fontSize:'10px', color:C.text, wordBreak:'break-all', margin:0 }}>{newReissueUrl}</p>
                              <p style={{ fontFamily:BODY, fontSize:'10px', color:C.textMuted, margin:'10px 0 0' }}>Импортируйте новую ссылку на ваших устройствах.</p>
                            </div>
                          )}
                        </div>
                      )}
                      {reissueError && (
                        <p style={{ fontFamily:BODY, fontSize:'11px', color:C.error, margin:'0' }}>{reissueError}</p>
                      )}
        ...
        {confirmReissue && (
          <div style={{ position:'fixed', inset:0, zIndex:20, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
            <div style={{ width:'100%', maxWidth:'420px', borderRadius:'24px', background:'#111', padding:'22px', boxShadow:'0 24px 80px rgba(0,0,0,0.35)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'14px' }}>
                <span style={{ width:'34px', height:'34px', borderRadius:'14px', background:'#FFB3B3', display:'grid', placeItems:'center', color:'#7A1B1B', fontSize:'18px' }}>⚠️</span>
                <div>
                  <p style={{ fontFamily:DISPLAY, fontSize:'16px', fontWeight:800, color:'#fff', margin:0 }}>Перевыпуск ссылки подписки</p>
                  <p style={{ fontFamily:BODY, fontSize:'12px', color:'#D1D5DB', margin:'4px 0 0' }}>После подтверждения все текущие подключения отзовутся.</p>
                </div>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginBottom:'18px' }}>
                {[
                  ...
                  <p key={idx} style={{ fontFamily:BODY, fontSize:'12px', color:'#E5E7EB', margin:'0', lineHeight:1.5 }}>
                    • {line}
                  </p>
                ))}
              </div>
              <p style={{ fontFamily:BODY, fontSize:'12px', color:'#F8FAFC', margin:'0 0 18px' }}>Вы уверены?</p>
              <div style={{ display:'flex', gap:'10px' }}>
                <button
                  onClick={() => setConfirmReissue(false)}
                  style={{ flex:1, padding:'12px 14px', borderRadius:'14px', border:'1px solid rgba(255,255,255,0.12)', background:'rgba(255,255,255,0.05)', color:'#F8FAFC', fontFamily:BODY, fontSize:'13px', fontWeight:700, cursor:'pointer' }}
                >
                  Вернуться назад
                </button>
                <button
                  ...
                  disabled={reissueLoading}
                  style={{ flex:1, padding:'12px 14px', borderRadius:'14px', border:'none', background:'#4A6EF5', color:'#fff', fontFamily:BODY, fontSize:'13px', fontWeight:700, cursor:reissueLoading ? 'not-allowed' : 'pointer' }}
                >
                  {reissueLoading ? 'Перевыпуск...' : 'Да, перевыпустить'}
                </button>
              </div>
            </div>
          </div>
