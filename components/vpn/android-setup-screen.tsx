...
interface AndroidSetupScreenProps { onNavigate: (screen: Screen) => void }

const steps = [
  {
    title: "Скачайте HApp",
    body: "Откройте Google Play и установите приложение HApp. Поддерживает VLESS, Shadowsocks и другие протоколы.",
    action: { label: "Скачать HApp в Google Play", url: "https://play.google.com/store/apps/details?id=com.happ.proxy" },
  },
  {
    title: "Скопируйте ключ подключения",
    body: "Вернитесь на главный экран ChampionVPN и нажмите «Настроить подключение» → «Скопировать ключ».",
    action: null,
  },
  {
    title: "Добавьте конфигурацию",
    body: "Откройте HApp, нажмите «+» → «Вставить из буфера обмена». Конфигурация добавится автоматически.",
    action: null,
  },
  {
    title: "Подключитесь",
    body: "Выберите добавленный сервер и нажмите «Подключить». Разрешите создание VPN-соединения. Готово!",
    action: null,
  },
]

export function AndroidSetupScreen({ onNavigate }: AndroidSetupScreenProps) {
    ...
    <div style={{ position:'relative', width:'100%', height:'100%', display:'grid', gridTemplateRows:'auto 1fr', background:C.bg, overflow:'hidden' }}>
      ...
      <main style={{ overflowY:'auto', scrollbarWidth:'none' as any, padding:`8px ${R.padH} 16px`, display:'flex', flexDirection:'column', gap:'10px', overscrollBehavior:'contain' }}>
        <div style={reveal(mounted, 1)}>
          ...
          <p style={{ fontFamily:BODY, fontSize:'13px', color:C.textMuted, margin:'0 0 12px' }}>Следуйте инструкции — займёт 2 минуты</p>
        </div>
        {steps.map((step, i) => (
