...
interface IosSetupScreenProps { onNavigate: (screen: Screen) => void }
...
const steps = [
  ...
  {
    title: "Скопируйте ссылку на подписку",
    body: "Вернитесь на главный экран ChampionVPN и нажмите кнопку «Ссылка на подписку» → «Копировать».",
    action: null,
  },
  {
  ...
  {
    title: "Подключитесь",
    body: "Выберите сервер из списка и нажмите «Подключить». Разрешите добавление VPN-конфигурации. Готово!",
    action: null,
  },
]

export function IosSetupScreen({ onNavigate }: IosSetupScreenProps) {
  ...
  return (
    <div style={{ borderRadius:R.cardRadius, background:C.cardLight, overflow:'hidden', ...reveal(mounted,idx) }}>
      <button onClick={onToggle}
        ...
        <span style={{ width:'28px', height:'28px', borderRadius:'50%', background:open?C.accent:C.accentSoft, color:open?'#fff':C.accent, fontFamily:DISPLAY, fontSize:'12px', fontWeight:800, display:'grid', placeItems:'center', flexShrink:0 }}>
          {index + 1}
        </span>
