  ...
  try {
    ...
    if (userResult.rows.length > 0) {
      ...
      if (isRemnawaveConfigured()) {
        try {
          // Fetch devices_count from subscription
          const subDevicesResult = await query(
            'SELECT devices_count FROM subscriptions WHERE id = $1',
            [payment.subscription_id]
          )
          const devicesCount = Math.max(1, parseInt(subDevicesResult.rows[0]?.devices_count) || 1)

          console.log('=== Creating VPN user in Remnawave ===')
          console.log('Telegram ID:', telegram_id)
          console.log('Duration (months):', payment.duration_months)
          console.log('Devices count:', devicesCount)
          console.log('Remnawave URL:', process.env.REMNAWAVE_API_URL)

          const vpnUser = await remnawave.createVpnUser(telegram_id, payment.duration_months, devicesCount)
