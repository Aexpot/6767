...
class RemnawaveClient:
    ...
    async def get_user_devices(self, telegram_id: int) -> List[Dict[str, Any]]:
        """Список HWID-устройств пользователя (GET /api/hwid/devices/{uuid})."""
        username = f"tg_{telegram_id}"
        try:
            raw = await self._get_raw_by_username(username)
    ...
    async def add_traffic_limit(self, telegram_id: int, gb: int) -> int:
        """Докупить ГБ белых списков: увеличить trafficLimitBytes на gb*1ГБ.

        Возвращает новый лимит в байтах (0 = безлимит).
        """
        username = f"tg_{telegram_id}"
        raw = await self._get_raw_by_username(username)
        current = int(raw.get("trafficLimitBytes") or 0)
        if current <= 0:
            return 0  # безлимит — докупка не требуется
        new_limit = current + int(gb) * (1 << 30)
        await self._update_raw_user(raw, {"data_limit": new_limit})
        return new_limit

    async def add_device_limit(self, telegram_id: int, count: int) -> int:
        """Докупить устройства: увеличить hwidDeviceLimit на count. Возвращает новый лимит."""
        username = f"tg_{telegram_id}"
        raw = await self._get_raw_by_username(username)
        current = int(raw.get("hwidDeviceLimit") or 0)
        new_limit = current + int(count)
        await self._update_raw_user(raw, {"device_limit": new_limit})
        return new_limit
