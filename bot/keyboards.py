...
def traffic_menu() -> InlineKeyboardMarkup:
    return rows(
        [btn(f"➕ Докупить ГБ ({BYPASS_PRICE_PER_GB} ₽/ГБ)", "buy_bypass_gb")],
        [btn("← Назад", "subscription")],
    )


def topup_gb_menu() -> InlineKeyboardMarkup:
    """Докупка ГБ белых списков. callback: topup_gb:{gb}:{price}."""
    rows_: list[list[InlineKeyboardButton]] = []
    for extra_gb, price in BYPASS_EXTRAS:
        if extra_gb <= 0:
            continue
        rows_.append([btn(f"+{extra_gb} ГБ — {price} ₽", f"topup_gb:{extra_gb}:{price}")])
    rows_.append([btn("← Назад", "traffic")])
    return rows(*rows_)


def topup_device_menu() -> InlineKeyboardMarkup:
    """Докупка устройств. callback: topup_device:{count}:{price}."""
    rows_: list[list[InlineKeyboardButton]] = []
    for count in (1, 2, 3, 5):
        price = count * DEVICE_EXTRA_PRICE
        word = "устройство" if count == 1 else "устройства"
        rows_.append([btn(f"+{count} {word} — {price} ₽", f"topup_device:{count}:{price}")])
    rows_.append([btn("← Назад", "devices")])
    return rows(*rows_)
...
def devices_menu() -> InlineKeyboardMarkup:
    return rows(
        [btn(f"➕ Докупить устройство (+{DEVICE_EXTRA_PRICE} ₽)", "buy_device")],
